  ...
  var Tom_N_Toms_franchisee ={
    ...
    section5:function(){
      var s5 = $('#section5'),
          ...
          step_span_bot_first = s5.find('.step_inner .bottom > div:nth-child(3)'),
      ...
      $(window).scroll(function(){
        if(load == false){
          if($(window).scrollTop() > $('#section3').offset().top+100){
            ...
            setInterval(function(){
              if(step_span_top_last.find('div').hasClass('show') && cnt2 > -1){
                cnt2--
              }
              stepFn2()
            },150)
      ...
      function stepFn(){
        step_span_top.eq(cnt).find('div').addClass('show')
      }
      function stepFn2(){
        step_span_bot.eq(cnt2).find('div').addClass('show')
      }
      $(window).scroll(function(){
        if($(this).scrollTop() > $('#section4').offset().top){
          if(step_span_bot_first.find('div').hasClass('show')){
            s5.addClass('show')
          }
        }
      })
